...
//                        setUrl(response.body().getResult().get(0).getFileName());
                        ...
                        String uri = Uri.encode(filename);
                        String url = BASE_URL+"uploads/laporan/"+uri;

                        Log.d("urlDownload", "onResponse: "+url);
                        downloadUtil = new DownloadUtil(PdfActivity.this);

                        downloadUtil.startPrDOwnloader(url, filename);
                    }
                }
