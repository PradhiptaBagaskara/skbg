...
class M_pdf extends CI_Model {
	...
	function laporan_proyek($idProyek)
	{
        ...
        foreach ($tx as $key) {
            ...
            $pdf->Cell(15,$heigh,strtoupper(str_replace('h','',$key->jenis)),1,0,'C');
...
###########
        ...
        $pdf->Cell(20,5,str_replace('h','',$ke->jenis),1,0,'C');
...
##################################################################
	    ...
	    $pdf->AddPage();
	    $pdf->AcceptPageBreak();
        $pdf->SetFooterInfo("Laporan Histori Kas - ".$user->nama);
